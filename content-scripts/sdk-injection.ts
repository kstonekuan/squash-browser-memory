...
import { logger } from '../src/utils/shared-utils';

// Type definitions for the injected API
interface SquashExtensionAPI {
  requestPermission: (appInfo: { appName: string; appId: string }) => Promise<{ granted: boolean; isFirstTime: boolean }>;
  getContext: (options?: { relevanceQuery?: string; timeRange?: string; maxTokens?: number; format?: string }) => Promise<{ success: boolean; data?: any; error?: string }>;
    ...
    getContext: async (options = {}) => {
      try {
        ...
        if (response.success) {
          ...
          return { success: false, error: response.error || 'Failed to get context' };
        }
      } catch (error) {
        logger.error('Error getting context:', error);
        return { success: false, error: 'Failed to communicate with extension' };
      }
    },
