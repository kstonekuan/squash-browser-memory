import { generateMockContext, generateMockResult } from "../mock-data";
	...
	describe("generateMockContext()", () => {
		...
		it("should filter by relevance query", () => {
			...
			const hasReactContent =
				...
				context.recentActivities.some((a) =>
					a.activity.toLowerCase().includes("react"),
				);
		...
		it("should handle multiple relevance query terms", () => {
			const context = generateMockContext({
				relevanceQuery: "typescript, api",
			});

			// Should have content related to TypeScript or API
			const hasRelevantContent = context.topics.some(
				(t) =>
